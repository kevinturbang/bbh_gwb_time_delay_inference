...
def plot_corner(fig,plot_data,color,hist_alpha=0.7,bins=20,labelsize=14,logscale=False,vmax=None):
    ...
    * `label`: A latex string for axis labeling
    ...
    for i,key in enumerate(keys):
        ...
        ax.set_title(r"${0:.2f}^{{+{1:.2f}}}_{{-{2:.2f}}}$".format(*getBounds(plot_data[key]['data'])),fontsize=20)
        ...
        if i==ndim-1:
            ax.set_xlabel(plot_data[key]['label'],fontsize=20)
        ...
        # If not the last dimension, loop across other variables and fill in the rest of the column with 2D plots
            ...
            for j,k in enumerate(keys[i+1:]):
                ...
                if i==0:
                    ax.set_ylabel(plot_data[k]['label'],fontsize=20)
                    ax.tick_params(axis='both', which='major', labelsize=19)
                    ax.tick_params(axis='both', which='minor', labelsize=19)
                else:
                    ax.set_yticklabels([])
...
def plot_corner_plus_hist_contour(fig,plot_data,hist_data,color,hist_alpha=0.7,bins=20,labelsize=14,logscale=False,vmax=None):
    ...
    * `label`: A latex string for axis labeling
    ...
    for i,key in enumerate(keys):
        ...
        ax.hist(hist_data[key]['data'],bins=np.linspace(hist_data[key]['plot_bounds'][0],hist_data[key]['plot_bounds'][1],bins),\
                histtype='step',color='black',lw=2,ls='dashed',density=True,zorder=2)
